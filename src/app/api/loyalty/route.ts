...
import { handleAddPointsRequest, handleGetLoyaltySummaryRequest } from '@/lib/api/loyalty-handler'
...
export async function GET() {
  ...
  return handleGetLoyaltySummaryRequest(client!.orgId, client!.supabase as unknown as ConfigClient)
}

export async function POST(request: NextRequest) {
  const { response, client } = await authClient()
  if (response) return response
  return handleAddPointsRequest(
    client!.orgId,
