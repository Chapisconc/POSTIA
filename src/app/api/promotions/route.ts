...
} from '@/lib/api/promotions-handler'
...
export async function GET() {
  ...
  return handleListPromotionsRequest(client!.orgId, client!.supabase as unknown as ConfigClient)
}

export async function POST(request: NextRequest) {
  const { response, client } = await authClient()
  if (response) return response
  return handleCreatePromotionRequest(
    client!.orgId,
