...
import { handleListCajaRequest, handleOpenCajaRequest } from '@/lib/api/caja-handler'
...
async function authClient() {
  ...
  try {
    ...
    return { response: null, client: { supabase: client, orgId, userId: user.id } }
  } catch (error) {
    return {
...
export async function GET() {
  ...
  return handleListCajaRequest(client!.orgId, client!.supabase as unknown as ConfigClient)
}

export async function POST(request: NextRequest) {
  const { response, client } = await authClient()
  if (response) return response
  return handleOpenCajaRequest(
    client!.orgId,
