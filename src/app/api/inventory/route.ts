...
} from '@/lib/api/inventory-handler'
...
export async function GET() {
  ...
  return handleListInventoryRequest(client!.orgId, client!.supabase as unknown as ConfigClient)
}

export async function POST(request: NextRequest) {
  const { response, client } = await authClient()
  if (response) return response
  return handleCreateMovementRequest(
    client!.orgId,
